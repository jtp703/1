...
public class Practica01JUnit5 {
	...
	public void testDevice(){
 		...
 		Device d1 = new Device("     iPhone 13"); //Cuidado con los espacios en blanco detrÃ¡s y delante del texto --> uso del metodo trim()
	...
	public void testDevicesSendMessagesRemove() {
 		...
 		String[] salidaEsperada = {"hola", "amiga", "que", "tal", "amigo", "como", "estamos"}; //Se eliminan palabras repetidas; todas en minÃºsculas
	...
	public void testUserDevices() {
		...
		assertTrue(u1.getNumDevices() == 3); //Y segun esto, el usuario u1 solo tiene 3 dispositivos...Â¿Â¿Â¿????????????
	
		for (int i=0; i<1000; i++) {
			u2.addDevices(new Device("__"));
		}
	
		assertTrue(u2.getNumDevices() == 1000); //Â¿Â¿???
		...
		//assertFalse(u1.contains("mensaje"));
